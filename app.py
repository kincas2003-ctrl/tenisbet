...
# Função para carregar a Agenda do Calendário
@st.cache_data
def load_agenda():
    # Tenta ler um ficheiro real chamado agenda.csv
    if os.path.exists("agenda.csv"):
        try:
            df_agenda = pd.read_csv("agenda.csv")
            # Converte a coluna para o formato de data puro do Python
            df_agenda['Data'] = pd.to_datetime(df_agenda['Data']).dt.date
            return df_agenda
        except:
            pass
            
    # Se não houver ficheiro, gera um Mockup Automático ancorado no dia de HOJE
...
# --- 6. ABAS DE TRABALHO ---
tab4, tab3, tab1, tab2 = st.tabs(["📅 Agenda", "🤖 Auto-Scanner", "🔍 Calculadora Manual", "🚀 CSV em Massa"])
...
# ==========================================
# ABA 4: AGENDA DE TORNEIOS (COM CALENDÁRIO)
# ==========================================
with tab4:
    st.header("📅 Calendário de Torneios")
    st.markdown("Seleciona a data abaixo. Podes alimentar esta lista criando um ficheiro `agenda.csv` na mesma pasta da app.")
    
    # Widget de Calendário Real
    data_selecionada = st.date_input("🗓️ Selecionar Data", datetime.today().date())
    
    df_agenda = load_agenda()
    
    # Filtrar pelos jogos da data escolhida
    jogos_do_dia = df_agenda[df_agenda['Data'] == data_selecionada]
    
    if jogos_do_dia.empty:
        st.info(f"Sem jogos agendados para a data selecionada ({data_selecionada.strftime('%d/%m/%Y')}).")
    else:
        torneios = jogos_do_dia['Torneio'].unique()
        for torneio in torneios:
            with st.expander(f"🏆 {torneio}", expanded=True):
                jogos_torneio = jogos_do_dia[jogos_do_dia['Torneio'] == torneio]
                for idx, jogo in jogos_torneio.iterrows():
                    ...
                    if col_btn.button("Carregar Jogo", key=f"btn_agenda_{idx}"):
                        ...
                        st.success(f"✅ Jogo carregado! Vai à aba **Auto-Scanner** ou **Calculadora Manual**.")
        ...
        if scan_p1 == scan_p2:
            ...
            with st.spinner("A simular e a analisar todas as linhas detetadas..."):
                ...
                for hcp_linha, odd in mercados_extraidos['set1_handicap']['P1'].items():
                    ...
                    lista_ev.append({"Mercado": f"Handicap 1º Set {scan_p1} ({hcp_linha})", "Prob": prob, "Odd": odd, "EV": (odd * prob) - 1})
